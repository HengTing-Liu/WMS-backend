...
CREATE TABLE inv_location (
    ...
    -- 库位等级与类型（完全自定义）
    location_grade      VARCHAR(50)             NOT NULL COMMENT '库位等级编码，用户自定义输入',
    location_type       VARCHAR(50)             NOT NULL COMMENT '库位类型名称，用户自定义输入（如：冰箱/层/架/行/盒/箱/孔等）',
    ...
    location_sort_no    VARCHAR(50)             NOT NULL COMMENT '排序号（用于树形展示排序，如 L0001001001）',
    ...
    INDEX idx_fullpath (location_fullpath_name)
...
VALUES (NULL, 'Type', '冰箱', 1, 5, 1, 1, 'REF001', '卧式冰箱', 'Inven01-1', 
